...
.PARAMETER Artifacts
    ...
    specified, all collectors are executed. If "runtime" is explicitly listed,
    the runtime collector is executed with -Force.
...
Arguments:
  ...
  -Artifacts <string>
      ...
      explicitly listed, runtime is executed with -Force.
...
function Get-CollectorParameters {
    ...
    $requestedArtifacts = @(Get-RequestedArtifactNames)
    if (($requestedArtifacts -contains "runtime") -and ((Get-ArtifactNameFromScript -ScriptName ([System.IO.Path]::GetFileName($ScriptPath))) -eq "runtime")) {
